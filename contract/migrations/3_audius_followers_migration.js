...
module.exports = async (deployer, network, [defaultAccount]) => {
  if (network.startsWith("rinkeby")) {
    ...
    try {
      ...
      await deployer.deploy(AudiusFollowersDistributer, distributerInfoCids["kovan"], "0x01BE23585060835E02B77ef475b0Cc51aA1e0709")
    } catch (err) {
      console.error(err)
    }
    ...
    try {
      ...
      await deployer.deploy(AudiusFollowersDistributer, distributerInfoCids["kovan"], "0xa36085F69e2889c224210F603D836748e7dC0088")
    } catch (err) {
      console.error(err)
    }
    ...
    try {
      ...
      await deployer.deploy(AudiusFollowersDistributer, distributerInfoCids[network], LinkToken.address)
    } catch (err) {
      console.error(err)
    }
  } else {
    ...
    await deployer.deploy(AudiusFollowersDistributer, distributerInfoCids["live"], '0x0000000000000000000000000000000000000000')
  }
}
