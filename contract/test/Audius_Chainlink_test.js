...
contract("Audius", (accounts) => {
    ...
    context("with LINK", () => {
      ...
      context("sending a request to a specific oracle contract address", () => {
        xit("triggers a log event in the new Oracle contract", async () => {
          const tx = await cc.requestCheckingAddress(
            oc.address,
            jobId,
            "userAddress",
            "tokenAddress",
  ...
  describe("generateClaimKey", () => {
    it("returns a max value", async () => {
      const claimKey = (
        await cc.generateClaimKey(
          new web3.utils.BN("18446744073709551615"),
          new web3.utils.BN("18446744073709551615")
        )
      ).toString();
      expect(claimKey).to.equal(
  ...
  xdescribe("#fulfill", () => {
    ...
    context("when called by anyone other than the oracle contract", () => {
      it("does not accept the data provided", async () => {
        const [, , , , , responseBytes] = oracle.convertFufillParams(
          request,
          response,
          {
            from: oracleNode,
