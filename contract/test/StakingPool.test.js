...
describe("StakingPool", () => {
  ...
  describe("pauseStakingOf", () => {
    it("throw an error if sender is not a creator", async () => {
      await this.pool.addStakingList(alice, this.abctoken.address, false, {
        from: owner,
      });
      try {
    ...
    it("does modify if already paused", async () => {
      await this.pool.addStakingList(alice, this.abctoken.address, true, {
        from: owner,
      });
      await this.pool.pauseStakingOf(this.abctoken.address, { from: alice });
      expect(
        await this.pool.tokensStakingPaused(this.abctoken.address)
      ).to.equal(true);
    });
  ...
  describe("tokensTotalSupply", () => {
    ...
    it("returns 0 when token is not registered", async () => {
      expect(
        (await this.pool.tokensTotalSupply(this.xyztoken.address)).toString()
      ).to.equal("0");
    });

    it("incremented by staking", async () => {
      expect(
        (await this.pool.tokensTotalSupply(this.abctoken.address)).toString()
      ).to.equal("0");
  ...
  describe("addStakingList", () => {
    ...
    it("register token", async () => {
      expect(await this.pool.registeredTokens(this.abctoken.address)).to.equal(
        true
      );
    });

    it("register creator", async () => {
      expect(await this.pool.tokensCreator(this.abctoken.address)).to.equal(
        alice
      );
    });
    ...
    it("can not add token from non owner", async () => {
      try {
        await this.pool.addStakingList(bob, this.abctoken.address, {
          from: alice,
        });
        assert.fail();
      } catch (error) {
  ...
  describe("stake", () => {
    ...
    it("staked given amount token from message sender", async () => {
      ...
      expect(
        (await this.pool.balanceOf(alice, this.abctoken.address)).toString()
      ).to.equal("50");
    ...
    it("emit an event", async () => {
      await this.abctoken.transfer(alice, 100, { from: owner });
      await this.abctoken.approve(this.pool.address, 100, { from: alice });
      const receipt = await this.pool.stake(100, this.abctoken.address, {
        from: alice,
      });
  ...
  describe("withdraw", () => {
    ...
    it("emit an event", async () => {
      ...
      const receipt = await this.pool.withdraw(100, this.abctoken.address, {
        from: alice,
      });
