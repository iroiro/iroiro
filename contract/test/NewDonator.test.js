...
describe("FanToken", () => {
  ...
  describe("tokenDonateeList", () => {
    it("returns zero address", async () => {
      expect(await this.donator.tokenDonateeList(this.abctoken.address)).to.equal(constants.ZERO_ADDRESS)
    })

    it("returns creator address", async() => {
      await this.donator.setDonatee(this.abctoken.address, { from: alice })
      expect(await this.donator.tokenDonateeList(this.abctoken.address)).to.equal(alice)
      expect(await this.donator.tokenDonateeList(this.xyztoken.address)).to.equal(constants.ZERO_ADDRESS)
    })
  })
  ...
  describe("donate", () => {
    ...
    it("throw an error if donatee is not registered", async () => {
      try {
        await this.donator.donate(this.xyztoken.address, 10000, { from: owner })
    ...
    it("throw an error if amount is not approved", async () => {
      try {
        await this.donator.donate(this.abctoken.address, 10001, { from: owner })
        assert.fail("should not throw error")
      } catch(error) {
        expect(error.reason).to.equal("ERC20: transfer amount exceeds allowance")
        assert(true)
      }
    })
