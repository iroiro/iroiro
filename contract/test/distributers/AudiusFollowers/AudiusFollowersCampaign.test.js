...
describe("AudiusFollowersCampaign", () => {
  ...
  beforeEach(async () => {
    this.distributer = await Distributer.new("Audius Test Distributer", link, {
      from: owner,
    });
  ...
  describe("generateClaimKey", () => {
    it("returns claim key", async () => {
      expect(
        (await this.campaign.generateClaimKey(new BN("1"))).toString()
      ).to.equal("11");
    });
  });
  ...
  describe("isClaimable", () => {
    it("returns false if user is is not registered", async () => {
      expect(await this.campaign.isClaimable(owner, { from: owner })).to.equal(
        false
      );
    });
  ...
  describe("claim", () => {
    ...
    it("throws an error if user is not claimable", async () => {
      expectRevert(
        this.campaign.claim({ from: owner }),
