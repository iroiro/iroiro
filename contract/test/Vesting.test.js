...
describe("Vesting", () => {
  ...
  beforeEach(async () => {
    ...
    await this.abctoken.transfer(
      this.vesting.address,
      (totalSupply * 50) / 100,
      { from: owner }
    );
    assert(
      (await this.abctoken.balanceOf(this.vesting.address)).toString() ===
        "500000000"
    );
  });
  ...
  describe("remainingAmount", () => {
    it("returns current balance", async () => {
      ...
      expect(
        (await this.vesting.remainingAmount(this.abctoken.address)).toString()
      ).to.equal("500000000");
  ...
  describe("addVesting", () => {
    it("register token as vesting", async () => {
      expect(await this.vesting.vestingTokens(this.abctoken.address)).to.equal(
        false
      );
      await this.vesting.addVesting(
        this.abctoken.address,
    ...
    it("increment token amount", async () => {
      ...
      expect(
        (
          await this.vesting.tokensVestingAmount(this.abctoken.address)
        ).toString()
      ).to.equal("500000000");
    });
    ...
    it("register account as recipient", async () => {
      ...
      expect(
        await this.vesting.tokensRecipient(this.abctoken.address)
      ).to.equal(alice);
    });
  ...
  describe("redeem", () => {
    ...
    xit("redeem partial token", async () => {
      ...
      const recipient = await this.vesting.tokensRecipient(
        this.abctoken.address
      ); // alice

      const recipientBalance = (
        await this.abctoken.balanceOf(recipient)
      ).toNumber();
  ...
  describe("redeemableAmount", () => {
    it("returns partial token", async () => {
      ...
      await this.vesting.addVesting(xyztoken.address, alice, start, end, {
        from: owner,
      });
      expect(
        (await this.vesting.redeemableAmount(xyztoken.address)).toNumber()
      ).to.equal(0);

      const halfYear = 60 * 60 * 24 * 30 * 6;
      await time.increase(halfYear);
      const partialAmount = (
        await this.vesting.redeemableAmount(xyztoken.address)
      ).toNumber();
