...
describe("Audius", () => {
  ...
  beforeEach(async () => {
    ...
    this.audius = await Audius.new(this.factory.address, linkToken, {
      from: owner,
    });
    ...
    await this.sampleToken.transfer(this.audius.address, 20000, {
      from: alice,
    });
  });
  ...
  describe("nextTokenId", () => {
    ...
    it("is incremented after token is registered", async () => {
      ...
      await this.audius.addAudiusList(id, "SampleHash", 100, { from: alice });
  ...
  describe("remainingAmount", () => {
    it("returns deposit tokens num ", async () => {
      const remainingAmount = (
        await this.audius.remainingAmount(this.sampleToken.address)
      ).toNumber();
