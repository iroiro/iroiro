...
describe("TokenFactory", () => {
  ...
  beforeEach(async function () {
    ...
    this.factory = await TokenFactory.new(
      this.vesting.address,
      this.stakingPool.address,
      { from: owner }
    );
    await this.stakingPool.transferOwnership(this.factory.address, {
      from: owner,
    });
  ...
  it("returns a total address", async function () {
    expect((await this.factory.tokenOf(alice)).toString()).to.equal(
  ...
  describe("createToken", function () {
    ...
    it("throw an error invalid ratio", async function () {
      try {
        await this.factory.createToken(
          alice,
          "ValidRatioToken",
          "VLD",
          100000000000,
    ...
    it("add token address", async function () {
      ...
      expect((await this.factory.tokenOf(1)).toString()).to.not.equal(
        constants.ZERO_ADDRESS
      );
    });
    ...
    it("add creator token amount", async function () {
      ...
      expect((await this.factory.tokenAmountOf(alice)).toString()).to.equal(
        "1"
      );
    });
    ...
    it("add creator token address", async function () {
      ...
      expect((await this.factory.tokenAmountOf(alice)).toString()).to.not.equal(
        constants.ZERO_ADDRESS
      );
    });
    ...
    it("both of token and creator token address newly added is same", async function () {
      ...
      const newCreatorTokenAddress = (
        await this.factory.creatorTokenOf(alice, 1)
      ).toString();
    ...
    it("mint token with given supply", async function () {
      ...
      expect((await fanToken.totalSupply()).toString()).to.equal(
    ...
    it("transfer token to vesting", async function () {
      ...
      const creatorBalance = (
        await fanToken.balanceOf(this.vesting.address)
      ).toString(); // Vesting amount is for creator
    ...
    it("add vesting", async function () {
      ...
      expect(
        (await this.vesting.tokensVestingAmount(newTokenAddress)).toString()
      ).to.equal("40000000000");
      expect(await this.vesting.tokensRecipient(newTokenAddress)).to.equal(
        alice
      );
      const start = (
        await this.vesting.tokensVestingStart(newTokenAddress)
      ).toNumber();
      const end = (
        await this.vesting.tokensVestingEnd(newTokenAddress)
      ).toNumber();
      expect(end).to.equal(start + 5 * 365 * 24 * 60 * 60);
      expect(
        (await this.vesting.tokensLastUpdate(newTokenAddress)).toNumber()
      ).to.equal(start);
    });
    ...
    it("add staking", async function () {
      ...
      const newTokenAddress = await this.factory.tokenOf(1);
      expect(
        await this.stakingPool.tokensStakingPaused(newTokenAddress)
      ).to.equal(false);
    });
    ...
    xit("pool contract can mint a created token", async function () {
      ...
      expect((await aliceToken.balanceOf(alice)).toString()).to.equal(
        "50000000100"
      );
    });

    it("emit an event", async function () {
      const receipt = await this.factory.createToken(
        alice,
        "AliceToken",
        "ALC",
        100000000000,
