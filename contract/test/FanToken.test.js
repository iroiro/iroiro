...
describe("FanToken", () => {
  ...
  it("has a creatorTokenRatio", async () => {
    expect((await this.fanStockToken.creatorTokenRatio()).toNumber()).to.equal(
      50
    );
  ...
  it("mints a token", async () => {
    expect((await this.fanStockToken.totalSupply()).toString()).to.equal(
      "1000000000"
    );
    expect((await this.aliceToken.totalSupply()).toString()).to.equal(
      "2000000000"
    );
  });

  it("transferred a token to creator", async () => {
    expect((await this.fanStockToken.balanceOf(owner)).toString()).to.equal(
      "1000000000"
    );
    expect((await this.aliceToken.balanceOf(alice)).toString()).to.equal(
      "2000000000"
    );
  });
  ...
  it("minter can mint a token", async () => {
    ...
    expect((await this.fanStockToken.balanceOf(alice)).toString()).to.equal(
      "100"
    );
  });
});
