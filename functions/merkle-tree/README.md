...
```bash
merkle-tree $ sam logs -n StockCheckerFunction --stack-name merkle-tree  --tail
```
...
```bash
merkle-tree $ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name merkle-tree 
```
