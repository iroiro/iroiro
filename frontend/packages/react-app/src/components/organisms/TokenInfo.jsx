...
const TokenInfo = ({ token, stakingInfo, withdrawStakingToken, claimEarnedToken, approve, stakeToken, handleStakeInput, stakeValue }) => (
  <div>
    ...
    <Box bg="white" p={4}>
      ...
      <Flex style={{ justifyContent: "space-between", maxWidth: "600px"}}>
        <Box mt={2}>
          <Text>Your staking amount:</Text>
          <Flex mt={2} style={{ alignItems: "center"}}>
            <Text fontSize={5} fontWeight={"bold"} mr={3}>{stakingInfo.stakingAmount} {token.symbol}</Text>
        ...
        <Box mt={2}>
          ...
          <Flex mt={2} style={{ alignItems: "center"}}>
            <Text fontSize={5} fontWeight={"bold"} mr={3}>{stakingInfo.earned} {token.symbol}</Text>
