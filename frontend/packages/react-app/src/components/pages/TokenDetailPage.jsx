...
const TokenDetailPage = () => {
  ...
  const [getAccountToken, { loading, error, data }] = useLazyQuery(
    GET_ACCOUNT_TOKEN
  );
  ...
  useEffect(() => {
    ...
    getAccountToken({
      variables: {
        id: walletAddress.toLowerCase().concat("-").concat(tokenAddress),
      },
    });
  ...
  useEffect(() => {
    ...
    setToken({
      ...
      totalSupply: ethers.utils.formatUnits(
        data.accountToken.token.totalSupply,
        decimals
      ),
      decimals: decimals,
  ...
  useEffect(() => {
    ...
    const f = async () => {
      ...
      const stakingAmount = await staking.balanceOf(
        walletAddress,
        tokenAddress
      );
      ...
      const allowanceAmount = await fanToken.allowance(
        walletAddress,
        addresses.Staking
      );
      const stakingInfo = {
  ...
  const withdrawStakingToken = async (address) => {
    ...
    const bgStakingAmount = await ethers.BigNumber.from(
      ethers.utils.parseUnits(stakingInfo.stakingAmount),
      token.decimals
    );
